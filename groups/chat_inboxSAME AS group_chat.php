...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Roboto',sans-serif; -webkit-tap-highlight-color:transparent; }
        ...
        body.dark-mode .tick-delivered { color:#8696A0; }
    ...
    <div class="chat-container">
        <div class="messages-container" id="messagesContainer">
            <?php if (empty($messages)): ?>
                <div class="empty-chat"><i class="fas fa-comments"></i><h3>No messages yet</h3><p>Send a message to start the conversation</p></div>
            <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function adjustHeight() { messageInput.style.height = 'auto'; messageInput.style.height = Math.min(messageInput.scrollHeight, 100) + 'px'; }
    ...
    function sendMessage() {
        ...
        el.className = 'message sent'; el.id = tempId;
        el.innerHTML = `<div class="message-content">${escapeHTML(msg)}<div class="message-time">Just now <span class="message-tick tick-delivered">✓</span></div></div>`;
        ...
        try {
            ...
            socket.onmessage = function(event) {
                ...
                if (data.type === 'new_message') {
                    const m = data.message; const isSent = m.sender_id == currentUserId;
                    const el = document.createElement('div');
                    el.className = `message ${isSent ? 'sent' : 'received'}`;
                    if (m.id) el.setAttribute('data-msg-id', m.id);
                    const time = new Date(m.timestamp).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});
                    if (!isSent) {
                        el.innerHTML = `<div class="message-sender">${escapeHTML(m.sender_name||'')}</div><div class="message-content">${escapeHTML(m.message)}<div class="message-time">${time}</div></div>`;
                    } else {
                        el.innerHTML = `<div class="message-content">${escapeHTML(m.message)}<div class="message-time">${time} <span class="message-tick tick-delivered">✓</span></div></div>`;
                    }
                    const empty = document.querySelector('.empty-chat'); if (empty) empty.remove();
                    messagesContainer.appendChild(el); scrollToBottom();
                }
